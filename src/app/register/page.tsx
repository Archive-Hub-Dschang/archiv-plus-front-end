...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        toast("Inscription réussie", { description: "Votre compte a été créé avec succès." })
        router.push("/dashboard")
      } else {
        toast("Erreur d'inscription", { description: "Cet email est déjà utilisé." })
      }
    } catch (error) {
      console.log(error);
      toast("Erreur", {
    ...
    <VisitorLayout>
        <div className="flex flex-col justify-center w-full py-10 space-y-3 sm:w-[350px]">
          <div className="flex flex-col space-y-2 text-center text-blue-600">
            ...
            <p className="text-sm text-gray-500">Inscrivez-vous pour accéder à toutes les fonctionnalités</p>
          </div>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-4 md:px-0">
              <div className="flex flex-col gap-2">
                <Label htmlFor="name">Nom complet</Label>
                <Input id="name" type="text" placeholder="Jean Dupont" value={name} onChange={(e) => setName(e.target.value)} autoFocus required />
              </div>
              <div className="flex flex-col gap-2">
          ...
          <p className="text-center text-sm text-gray-500">
            ...
            <Link href="/login" className="font-medium underline-offset-4 hover:underline text-blue-600">Se connecter</Link>
          </p>
        </div>
    </VisitorLayout>
  )
