...
export function UserListItem({ name, id, startDate, endDate }: { name: string, id: number, startDate: string, endDate: string }) {
    return (
        <div className='grid grid-cols-[auto_auto_auto_auto] md:grid-cols-4 justify-items-start items-center border-b-2 rounded-sm border-gray-200 text-sm p-2 cursor-pointer hover:shadow-lg hover:hover:bg-blue-300'>
            <input type="checkbox" name={`user-${id}`} id={`user-${id}`} className='size-4'/>
