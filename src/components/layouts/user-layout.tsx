...
import {Download, Heart, Home, LogOut, Menu, ShoppingCart} from "lucide-react"
...
export function UserLayout({children}: UserLayoutProps) {
    ...
    return (
        <div className="flex flex-col h-screen">
            <header
                className="h-16 flex items-center justify-between px-4 md:px-12 sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
                {/* mobile navigation */}
                ...
                <div className="flex items-center gap-4 md:gap-6">
                    ...
                    <Link href="/profile" className="flex items-center gap- cursor-pointer">
                        <Avatar>
                        ...
                        <nav className="flex-1 space-y-2 py-2 px-8">
                            {navigation.map((item) => (
                                <Link key={item.name} href={item.href}
                                      className={cn("flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium", pathname === item.href ? "bg-primary text-primary-foreground" : "hover:bg-muted")}>
                ...
                <nav className="pt-6 flex flex-col text-nowrap space-y-1">
                    {navigation.map((item) => (
                        <Link href={item.href} key={item.name}
                              className={cn("flex items-center text-sm font-medium p-2 rounded-md", pathname === item.href ? "bg-blue-100 text-blue-600" : "text-gray-900 hover:bg-gray-100")}>
